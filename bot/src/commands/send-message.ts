...
export const command: SlashCommand = {
    ...
    .addStringOption(option =>
      option
        ...
        .addChoices(
          ...
          { name: 'DM (Mensagem Direta)', value: 'dm' }
        )
    )
    .addStringOption(option =>
      option
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (tipo === 'canal' && !canal) {
      await interaction.reply({
        embeds: [createErrorEmbed(
          ...
          'Você selecionou enviar para um canal, mas não especificou qual canal.'
        )],
        ephemeral: true
      });
      return;
    ...
    if (tipo === 'dm' && !usuario) {
      await interaction.reply({
        embeds: [createErrorEmbed(
          ...
          'Você selecionou enviar DM, mas não especificou qual usuário.'
        )],
        ephemeral: true
      });
      return;
    ...
    try {
      // Monta a mensagem
      let messageContent: { content?: string; embeds?: EmbedBuilder[] };
      ...
      if (tipo === 'canal' && canal) {
        ...
        await interaction.editReply({
          embeds: [createSuccessEmbed(
            'Mensagem Enviada',
            `Mensagem enviada com sucesso para <#${canal.id}>!`
          )]
        });
        ...
        try {
          ...
          await interaction.editReply({
            embeds: [createSuccessEmbed(
              'Mensagem Enviada',
              `Mensagem enviada com sucesso para ${usuario.tag}!`
            )]
          });
          ...
          await interaction.editReply({
            embeds: [createErrorEmbed(
              'Erro ao Enviar DM',
              `Não foi possível enviar DM para ${usuario.tag}. ` +
