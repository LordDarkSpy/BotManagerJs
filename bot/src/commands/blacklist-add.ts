...
export const command: SlashCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      if (existente) {
        await interaction.editReply({
          embeds: [createErrorEmbed(
            'Já na Blacklist',
            `${usuario.username} já está na blacklist.\nMotivo: ${existente.motivo}`
          )]
        });
        return;
      }
      ...
      const embed = createBlacklistAddedEmbed(
        usuario.username,
        motivo,
        interaction.user
      );
      ...
      try {
        await usuario.send({
          embeds: [createErrorEmbed(
            ...
            `Você foi adicionado à blacklist do servidor **${interaction.guild?.name}**.\n\n` +
            `**Motivo:** ${motivo}\n` +
