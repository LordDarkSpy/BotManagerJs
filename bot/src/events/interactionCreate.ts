...
async function handleSlashCommand(
  ...
  if (!command) {
    ...
    await interaction.reply({
      embeds: [createErrorEmbed(
        'Comando não encontrado',
        'Este comando não está mais disponível.'
      )],
      ephemeral: true
    });
    return;
  ...
  if (command.cooldown) {
    ...
    if (cooldownResult.onCooldown) {
      await interaction.reply({
        embeds: [createErrorEmbed(
          'Aguarde',
          `Você pode usar este comando novamente em ${cooldownResult.remaining} segundos.`
        )],
        ephemeral: true
      });
      return;
  ...
  try {
    ...
    const errorEmbed = createErrorEmbed(
      ...
      'Ocorreu um erro ao processar este comando. Tente novamente mais tarde.'
    );
 ...
 */
async function sendErrorResponse(interaction: Interaction, error: unknown): Promise<void> {
