...
let ausenciaTask: cron.ScheduledTask | null = null;
 ...
 */
async function checkAusenciaReturns(): Promise<void> {
  if (!discordClient) {
 ...
 */
async function notifyAusenciaReturn(
  ausencia: {
    id: string;
    guildId: string;
    membro: { discordId: string; nome: string };
  }
): Promise<void> {
 ...
 */
export async function forceCheckAusencias(): Promise<void> {
 ...
 */
export function scheduleNotification(date: Date, callback: () => void): NodeJS.Timeout {
  const now = new Date();
