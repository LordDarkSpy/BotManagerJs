...
import { 
  createSuccessEmbed, 
  createErrorEmbed,
  createSetRequestEmbed,
  createAusenciaRegisteredEmbed,
  createFgtsRegisteredEmbed,
 ...
 */
async function handleSetSubmit(interaction: ModalSubmitInteraction): Promise<void> {
  ...
  if (tipoRaw !== 'ADULTO' && tipoRaw !== 'BABY') {
    await interaction.reply({
      embeds: [createErrorEmbed(
        'Tipo Inválido',
        'O tipo deve ser **ADULTO** ou **BABY**.'
      )],
      ephemeral: true
    });
    return;
  ...
  const tipo = tipoRaw as 'ADULTO' | 'BABY';
  ...
  try {
    ...
    // Registra log
    await logSetSolicitado(
      guildId,
      userId,
      nome,
      JSON.stringify({ nome, telefone, idJogo, recrutador, tipo })
    );

    await interaction.editReply({
      embeds: [createSuccessEmbed(
        'Solicitação Enviada',
        'Sua solicitação de Set foi enviada com sucesso!\n\n' +
 ...
 */
async function handleSetRejectConfirm(
  ...
  try {
    ...
    try {
      await solicitante.send({
        embeds: [createErrorEmbed(
          'Set Reprovado',
          `Sua solicitacao de Set foi **reprovada**.\n\n**Motivo:** ${motivo}`
        )]
      });
    } catch {
 ...
 */
async function handleAusenciaSubmit(interaction: ModalSubmitInteraction): Promise<void> {
  const guildId = interaction.guildId!;
  const userId = interaction.user.id;
  ...
  if (!dataInicio || !dataFim) {
    await interaction.reply({
      embeds: [createErrorEmbed(
        'Data Inválida',
        'As datas devem estar no formato DD/MM/AAAA (ex: 25/12/2024).'
      )],
      ephemeral: true
    });
    return;
  ...
  try {
    ...
    if (config?.canalAusencia) {
      try {
        ...
        if (channel && channel instanceof TextChannel) {
          const embed = createAusenciaRegisteredEmbed(
            interaction.user,
 ...
 */
async function handleFgtsSubmit(interaction: ModalSubmitInteraction): Promise<void> {
  const guildId = interaction.guildId!;
  const userId = interaction.user.id;
  ...
  if (isNaN(ano) || ano < 2020 || ano > anoAtual + 1) {
    await interaction.reply({
      embeds: [createErrorEmbed(
        'Ano Invalido',
        `O ano deve ser um numero valido entre 2020 e ${anoAtual + 1}.`
      )],
      ephemeral: true
    });
    return;
  ...
  if (isNaN(valor) || valor <= 0) {
    await interaction.reply({
      embeds: [createErrorEmbed(
        'Valor Invalido',
        'O valor deve ser um numero positivo (ex: 100.00 ou 100,00).'
      )],
      ephemeral: true
    });
    return;
  ...
  try {
    ...
    if (pagamentoExistente) {
      await interaction.editReply({
        embeds: [createErrorEmbed(
          'Pagamento Duplicado',
          `Voce ja registrou o pagamento do FGTS para **${mes.toString().padStart(2, '0')}/${ano}**.\n\n` +
    ...
    if (config?.canalFgts) {
      try {
        ...
        if (channel && channel instanceof TextChannel) {
          const embed = createFgtsRegisteredEmbed(
            interaction.user,
            valor,
            idTransacao,
            mes,
    ...
    await interaction.editReply({
      embeds: [createSuccessEmbed(
        'FGTS Pago',
        `Seu pagamento de FGTS foi registrado com sucesso!\n\n` +
