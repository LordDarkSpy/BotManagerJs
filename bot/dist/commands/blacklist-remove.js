        ...
        try {
            ...
            if (!existente) {
                await interaction.editReply({
                    embeds: [createErrorEmbed('Não encontrado', `${usuario.username} não está na blacklist.`)]
                });
                return;
            }
            // Remove da blacklist
            ...
            // Registra log
            await logBlacklistRemove(guildId, interaction.user.id, interaction.user.username, usuario.id, usuario.username);
            ...
            try {
                await usuario.send({
                    embeds: [createSuccessEmbed('Removido da Blacklist', `Você foi removido da blacklist do servidor **${interaction.guild?.name}**.\n\n` +
...
//# sourceMappingURL=blacklist-remove.js.map