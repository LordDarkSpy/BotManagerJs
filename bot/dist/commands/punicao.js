 ...
 */
async function handleAplicarPunicao(interaction) {
    ...
    try {
        ...
        if (!membro) {
            await interaction.editReply({
                embeds: [createErrorEmbed('Membro nao encontrado', 'Este usuario nao esta cadastrado como membro.')]
            });
            return;
        }
        ...
        if (tiposPunicao.length === 0) {
            await interaction.editReply({
                embeds: [createErrorEmbed('Sem tipos de punicao', 'Nenhum tipo de punicao cadastrado. Use o dashboard para configurar os tipos de punicao.')]
            });
            return;
        }
        ...
        try {
            await targetUser.send({
                embeds: [createErrorEmbed(`Punicao: ${tipoPunicao.codigo}`, `Voce recebeu uma punicao.\n\n` +
        ...
        const acoesTexto = acoesRealizadas.length > 0
            ...
            : '';
        await interaction.editReply({
            embeds: [createSuccessEmbed('Punicao Aplicada', `**Membro:** ${membro.nome} (<@${targetUser.id}>)\n` +
 ...
 */
async function handleHistoricoPunicao(interaction) {
    const guildId = interaction.guildId;
    const targetUser = interaction.options.getUser('membro', true);
    await interaction.deferReply({ ephemeral: true });
    try {
        ...
        if (punicoes.length === 0) {
            await interaction.editReply({
                embeds: [createInfoEmbed('Historico Limpo', `**${membro.nome}** nao possui nenhuma punicao registrada.`)]
            });
            return;
        }
        ...
        await interaction.editReply({
            embeds: [createInfoEmbed(`Historico de ${membro.nome}`, `**Punicoes ativas:** ${ativas}\n` +
 ...
 */
async function handleRemoverPunicao(interaction) {
    const guildId = interaction.guildId;
    ...
    try {
        ...
        if (!punicao) {
            await interaction.editReply({
                embeds: [createErrorEmbed('Punicao nao encontrada', 'Esta punicao nao existe ou ja foi removida.')]
            });
            return;
        }
        // Remove a punicao
        ...
        await interaction.editReply({
            embeds: [createSuccessEmbed('Punicao Removida', `**Membro:** ${punicao.membro.nome}\n` +
 ...
 */
async function handleListarTipos(interaction) {
    const guildId = interaction.guildId;
    await interaction.deferReply({ ephemeral: true });
    try {
        ...
        if (tipos.length === 0) {
            await interaction.editReply({
                embeds: [createInfoEmbed('Tipos de Punicao', 'Nenhum tipo de punicao cadastrado.\n\n' +
        ...
        await interaction.editReply({
            embeds: [createInfoEmbed('Tipos de Punicao', `Tipos configurados (ordenados por nivel):\n\n${listaTipos}`)]
        });
    }
    catch (error) {
...
//# sourceMappingURL=punicao.js.map