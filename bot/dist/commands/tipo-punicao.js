 ...
 */
async function handleCadastrar(interaction, guildId) {
    ...
    try {
        ...
        await interaction.editReply({
            embeds: [createSuccessEmbed('Tipo de Punicao Cadastrado', `O tipo de punicao **${codigo}** foi cadastrado com sucesso!\n\n` +
                    `**Nome:** ${nome}\n` +
                    `**Nivel:** ${nivel}\n` +
...
async function handleListar(interaction, guildId) {
    ...
    try {
        ...
        if (tipos.length === 0) {
            await interaction.editReply({
                embeds: [createErrorEmbed('Nenhum Tipo Cadastrado', 'Nao ha tipos de punicao cadastrados.\n\n' +
        ...
        for (const tipo of tipos) {
            embed.addFields({
                ...
                value: `${tipo.descricao ? `${tipo.descricao}\n` : ''}` +
 ...
 */
async function handleRemover(interaction, guildId) {
    const codigo = interaction.options.getString('codigo', true).toUpperCase();
    await interaction.deferReply({ ephemeral: true });
    try {
        // Busca o tipo
        ...
        if (tipo._count.punicoes > 0) {
            await interaction.editReply({
                embeds: [createErrorEmbed('Nao e Possivel Remover', `O tipo **${codigo}** possui ${tipo._count.punicoes} punicao(oes) aplicada(s).\n` +
        ...
        await interaction.editReply({
            embeds: [createSuccessEmbed('Tipo Removido', `O tipo de punicao **${codigo}** foi removido com sucesso.`)]
        });
    }
    catch (error) {
...
//# sourceMappingURL=tipo-punicao.js.map