...
import { createErrorEmbed, createCargoChangeEmbed } from '../utils/embeds.js';
import { canManageCargos, canSetCargo, PermissionErrors } from '../utils/permissions.js';
 ...
 */
async function handleCargoSelectMember(interaction) {
    ...
    if (cargos.length === 0) {
        await interaction.reply({
            embeds: [createErrorEmbed('Sem Cargos', 'Não há cargos cadastrados. Use `/cargo-cadastrar` para criar cargos.')],
            ephemeral: true
        });
        return;
    }
 ...
 */
async function handleCargoSetCargo(interaction, membroId) {
    const guildId = interaction.guildId;
    const member = interaction.member;
    ...
    try {
        ...
        await interaction.editReply({
            content: null,
            embeds: [createCargoChangeEmbed(membro.nome, cargoAntigoNome, novoCargo.nome, interaction.user)],
            components: []
        });
    }
    catch (error) {
...
//# sourceMappingURL=selectMenus.js.map